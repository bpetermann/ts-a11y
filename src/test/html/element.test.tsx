...
import { canHaveAriaHidden } from '../../diagnostics/html/element';
...
suite('Element Test Suite', () => {
  test("Should allow 'aria-hidden' on a plain <div> element", async () => {
    ...
    assert.strictEqual(canHaveAriaHidden(getNode(content)), true);
  });

  test("Should not allow 'aria-hidden' on a <button> element", async () => {
    const content = '<button></button>';

    assert.strictEqual(canHaveAriaHidden(getNode(content)), false);
  });
